...
export function WorkSection() {
  return (
    <section id="work" className="section-block section-divider scroll-mt-16">
          ...
          <StaggerItem key={project.id}>
            <article className="group grid gap-6 py-10 md:grid-cols-12 md:items-start md:gap-8 md:py-12">
              <div className="md:col-span-1">
                <span className="font-mono text-sm tracking-wider text-ink-faint">
                  {project.id}
                </span>
              </div>

              <div className="md:col-span-3">
                <h3 className="serif-display text-2xl transition-opacity group-hover:opacity-70 md:text-3xl">
                  {project.title}
                </h3>
                <p className="label-caps mt-2">{project.tags.join(" · ")}</p>
              </div>
